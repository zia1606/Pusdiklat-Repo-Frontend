  ...
  <div class="bg-gray-50 min-h-screen">
        ...
        <div class="p-10">
          <div class="max-w-6xl mx-auto">
            <div class="flex flex-col sm:flex-row justify-between items-center mb-10 gap-4">
              <div>
                <h2 class="text-2xl font-black text-gray-900 tracking-tight">Tata Tertib</h2>
                <p class="text-[11px] font-bold text-gray-400 uppercase tracking-[0.2em] mt-1">Manajemen Peraturan</p>
              </div>
              ...
              <table class="w-full text-left border-collapse">
                <thead>
                  <tr class="bg-gray-50/50 border-b border-gray-50">
                    <th class="py-5 px-6 text-[11px] font-bold text-gray-400 uppercase tracking-widest w-20">Urutan</th>
                    <th class="py-5 px-6 text-[11px] font-bold text-gray-400 uppercase tracking-widest">Peraturan</th>
                    <th class="py-5 px-6 text-[11px] font-bold text-gray-400 uppercase tracking-widest w-24">Status</th>
                ...
                <tbody class="divide-y divide-gray-50">
                  <tr v-for="item in items" :key="item.id" class="hover:bg-blue-50/30 transition-all">
                    <td class="py-6 px-6 text-sm font-bold text-gray-600">{{ item.order_number }}</td>
                    <td class="py-6 px-6 text-sm text-gray-900">{{ item.rule_text }}</td>
                    <td class="py-6 px-6">
                    ...
                    <td class="py-6 px-8 text-right flex justify-end space-x-2">
                      ...
                      <button @click="confirmDelete(item.id)" class="h-9 w-9 rounded-xl flex items-center justify-center bg-gray-50 text-gray-400 hover:bg-red-600 hover:text-white transition-all"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                    </td>
                  </tr>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm">
      <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
        <div class="bg-blue-600 px-8 py-6"><h3 class="text-lg font-black text-white">{{ isEditMode ? 'Edit Tata Tertib' : 'Tambah Tata Tertib' }}</h3></div>
        <div class="p-8">
          <form @submit.prevent="saveData" class="space-y-4">
            <div>
              <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Peraturan</label>
              <textarea v-model="form.rule_text" required rows="3" class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl focus:border-blue-500 focus:outline-none"></textarea>
            </div>
            <div>
            ...
            <div class="flex items-center mt-2">
              ...
              <label for="isActive" class="ml-2 text-sm font-bold text-gray-900">Aktifkan Peraturan</label>
            </div>
...
useHead({ title: 'Manajemen Tata Tertib - Sistem Repositori' });
...
const fetchData = async () => {
  try {
    const response = await $fetch(`${apiBaseUrl}/api/tata-tertib`, {
...
const openAddModal = () => {
  ...
  form.value = { id: null, rule_text: '', order_number: items.value.length + 1, is_active: true };
...
const saveData = async () => {
  ...
  try {
    ...
    const url = isEditMode.value ? `${apiBaseUrl}/api/tata-tertib/${form.value.id}` : `${apiBaseUrl}/api/tata-tertib`;
...
const deleteData = async () => {
  ...
  try {
    await $fetch(`${apiBaseUrl}/api/tata-tertib/${itemToDelete.value}`, {
      method: 'DELETE',
      headers: { 'Authorization': `Bearer ${authStore.token}` }
    });
