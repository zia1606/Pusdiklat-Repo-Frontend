          ...
          <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="lg:w-2/3">
              <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 relative">
                ...
                <!-- Category and Badges - Improved Layout -->
                <div class="flex flex-wrap items-center gap-2 mb-4">
                  ...
                  <!-- Format Badge (PDF/YouTube) - Moved to metadata section -->
                </div>
...
>
  ...
  Buka di YouTube
                ...
                <!-- Metadata - Improved Layout with better alignment -->
                <div class="mb-6">
    ...
    <div class="flex flex-row">
      ...
      <div class="w-1/2 md:w-3/4 text-gray-900">: {{ koleksi.keywords || '-' }}</div>
                ...
                <div class="flex flex-wrap gap-3 pt-4 border-t border-gray-200">
                  <!-- Show PDF button only if PDF exists and no YouTube -->
                  <!-- Show PDF button only if PDF exists and no YouTube -->
<button 
  ...
  Baca Dokumen
            ...
            <div class="lg:w-1/3">
              ...
              <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-200 sticky top-5">
                <h2 class="text-lg font-bold text-gray-900 mb-4 pb-2 border-b border-gray-200">Rekomendasi Lainnya</h2>
                <!-- Error Message -->
...
const koleksiToDelete = ref(null);
    ...
    //   { id: 2, title: "Pemanfaatan Big Data dalam Sektor Publik", author: "John Smith" },
    //   { id: 3, title: "Machine Learning untuk Statistik", author: "Alice Brown" }
    // ]);
    // Data states
const recommendedDocs = ref([]);
...
const fetchRecommendations = async () => {
  try {
    ...
    const response = await axios.get(`${apiBaseUrl}/api/koleksi/${route.params.id}/recommendations`);

    console.log('API Response:', response.data); // Debugging

    if (response.data.success) {
      const similarCollections = response.data.data.similar_collections;
      let recommendations = [];

      // Handle kedua struktur response
      if (Array.isArray(similarCollections.recommendations)) {
        // Format ketika tidak ada preprocessing (langsung array)
        recommendations = similarCollections.recommendations;
      } else if (similarCollections.recommendations?.recommendations) {
        // Format ketika ada preprocessing (nested object)
        recommendations = similarCollections.recommendations.recommendations;
      }
      ...
      recommendedDocs.value = recommendations.map(item => ({
        ...
        kategori: item.kategori || item.kategoriBangKom || item.kategori_bang_kom_id || '-',
        jenis_dokumen: item.jenis_dokumen || item.jenisDokumen || item.jenis_dokumen_id || '-',
        tahun_terbit: item.tahun_terbit,
        views: item.views || 0,
        similarity_score: item.similarity_score || 
                         (similarCollections.recommendation_type === 'content_based' ? 0.8 : 0.7)
      }));

      if (recommendedDocs.value.length === 0) {
...
// Fallback jika rekomendasi kosong/error
const fetchFallbackRecommendations = async () => {
  try {
    ...
    if (koleksi.value.kategori_bang_kom_id) {
      ...
      if (response.data.data && response.data.data.length > 0) {
        recommendedDocs.value = response.data.data.map(item => ({
          ...
          tahun_terbit: item.tahun_terbit,
          views: item.views,
          similarity_score: 0.5 // Lower score untuk fallback
        }));
        return;
      }
    }
    ...
    const latestResponse = await axios.get(`${apiBaseUrl}/api/koleksi`, {
      params: {
        limit: 5,
        sort_by: 'terbaru',
        exclude: route.params.id
    ...
    if (latestResponse.data.data) {
      recommendedDocs.value = latestResponse.data.data.map(item => ({
        id: item.id,
        judul: item.judul,
        penulis: item.penulis,
        kategori: item.kategoriBangKom?.nama || '-',
        ...
        tahun_terbit: item.tahun_terbit,
        views: item.views,
        similarity_score: 0.3 // Lowest score untuk fallback terakhir
      }));
    }
  } catch (error) {
    console.error('Error fetching fallback recommendations:', error);
    recommendedDocs.value = []; // Tetap kosong jika semua gagal
  }
};
...
// Fetch YouTube data - Enhanced with better debugging
const fetchYoutubeData = async () => {
  ...
  try {
    ...
    if (embedId) {
      ...
      youtubeError.value = null; // Clear error since fallback worked
    }
  } finally {
...
//     try {
//       await axios.post(`${apiBaseUrl}:8000/api/riwayat-baca', {
//         koleksi_id: id
//       }, {
//         headers: { Authorization: `Bearer ${authStore.token}` }
//       });
//     } catch (error) {
//       // Jika sudah ada di riwayat, tidak masalah
...
const viewPdf = async (id) => {
  ...
  try {
    ...
    // Buka halaman PDF Viewer di tab baru
    const timestamp = Date.now();
    const pdfViewerUrl = `/detail/pdf-view/${id}?t=${timestamp}`;
...
const shareViaTelegram = () => {
  ...
  //       await axios.delete(`${apiBaseUrl}:8000/api/simpan-koleksi/by-koleksi/${koleksiId}`, {
  //         headers: { Authorization: `Bearer ${authStore.token}` }
  //       });
  //       savedItems.value[koleksiId] = false;
  //       showToast('success', 'Koleksi dihapus dari simpan');
  //     } else {
  //       await axios.post(`${apiBaseUrl}:8000/api/simpan-koleksi', { koleksi_id: koleksiId }, {
  //         headers: { Authorization: `Bearer ${authStore.token}` }
  //       });
...
watch(() => route.params.id, async (newId) => {
  ...
  /* YouTube responsive container - Fixed version */
.youtube-container {
  position: relative;
  width: 100%;
  height: 0;
