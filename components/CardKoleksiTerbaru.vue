      ...
      >
        <div class="bg-white rounded-lg shadow-sm overflow-hidden h-full flex flex-col hover:shadow-md transition-all duration-300 border border-gray-200 group hover:border-blue-200 relative">
          ...
          <NuxtLink :to="`/detail/${card.id}`" class="block aspect-[2/3] w-full overflow-hidden relative group-hover:opacity-90 transition-opacity">
            ...
            />
            <div v-else class="w-full h-full bg-gradient-to-br from-blue-50 to-blue-100 flex flex-col items-center justify-center text-blue-300">
              ...
              <span class="text-[9px] mt-2 font-bold uppercase tracking-widest opacity-40">No Cover</span>
            </div>
          </NuxtLink>
          ...
          <div class="p-3.5 flex-1 flex flex-col">
            ...
            <div class="flex flex-wrap gap-2 mb-3">
              <span v-if="card.kategori" class="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-bold bg-blue-100 text-blue-800 uppercase">
                {{ card.kategori }}
              </span>
              <span v-if="card.jenisDokumen" class="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-bold bg-green-100 text-green-800 uppercase">
                {{ card.jenisDokumen }}
              </span>
              <!-- Status Badge -->
              <span v-if="card.is_active === 1" class="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-black bg-red-100 text-red-600 uppercase tracking-tighter">
                Active
              </span>
            ...
            <NuxtLink 
              :to="`/detail/${card.id}`" 
              class="block text-sm font-bold text-gray-900 hover:text-blue-600 transition-colors duration-200 mb-1 line-clamp-2 leading-tight"
            >
              {{ card.title }}
            </NuxtLink>
            ...
            <!-- Author and Year -->
            <div class="flex items-center text-[10px] text-gray-400 my-2 space-x-3">
              <div class="flex items-center">
                <svg class="w-3 h-3 mr-1 text-blue-600" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
            ...
            <!-- Detail button removed per user request -->
          </div>
        </div>
      </div>
    </div>
...
const { apiFetch } = useApi();
  ...
  // KONFIGURASI
  const TOTAL_CARDS = 12; // Menampilkan lebih banyak kartu
  const CARDS_PER_SLIDE = {
    small: 1,   // <470px
    medium1: 2, // 470-775px
    medium2: 4, // 775-1024px
    large: 6    // ≥1024px
  };
  ...
  const updateCardsPerSlide = () => {
    ...
    else if (width < 1200) cardsPerSlide.value = CARDS_PER_SLIDE.medium2;
  ...
  const getKoleksiTerbaru = async () => {
    try {
      ...
      const res = await apiFetch('/koleksi/filter', {
        params: { 
          sort_by: 'terbaru',
