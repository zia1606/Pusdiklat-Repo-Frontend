    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-middle bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full border border-gray-100">
        <div class="bg-gradient-to-r from-blue-900 to-blue-800 px-6 py-4 flex justify-between items-center">
          <h3 class="text-lg font-bold text-white" id="modal-title">Tambah Penulis Baru</h3>
        ...
        <form @submit.prevent="submit">
          <div class="px-6 py-6 space-y-4">
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-1">Nama Penulis</label>
              <input 
                v-model="form.nama" 
                type="text" 
                placeholder="Masukkan nama lengkap penulis..."
                required
                class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all shadow-sm"
              />
          ...
          <div class="bg-gray-50 px-6 py-4 flex flex-row-reverse gap-3 border-t border-gray-100">
            <button 
              type="submit" 
              :disabled="isLoading"
              class="px-6 py-2 bg-blue-700 hover:bg-blue-800 text-white rounded-xl font-bold text-sm shadow-lg shadow-blue-200 transition-all active:scale-95 disabled:opacity-50"
            >
              <span v-if="isLoading">Menyimpan...</span>
              <span v-else>Simpan Penulis</span>
            </button>
            <button 
              @click="close" 
              type="button" 
...
const submit = async () => {
  ...
  try {
    ...
    showToast('success', 'Penulis berhasil ditambahkan!');
    emit('success', res.data);
    close();
  } catch (err) {
    console.error(err);
    error.value = err.response?._data?.message || 'Gagal menambahkan penulis. Nama mungkin sudah ada.';
